%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%                    Ditmar Halla (Matriculation number: )              %%%%%%%%%%%%%%%%%%%%%%%%%%%%               
%%%%%%%%%%%%%%%%%%%%%%%%%%%%                                   Report 1                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%                                  
...
x = 0:0.3:20;                       %define "x" from star to finish and the setp count
y = log10(x)+cos(x)-1;              %function
plot(x,y,'r-o')                     %ploting the function
title('f(x) = log(x)+cos(x)-1'),    %adding a title
xlabel('value of x')                %labeling
ylabel('value of y')
...
fprintf('The root using the "fzero" function are:  1: %.10f    2: %.10f    3: %.10f \n',r1_fzero,r2_fzero,r3_fzero);
                                                    %We display the results using the fprintf
                                    
plot(r1_fzero,0,'bo')                               %We show the roots we found in the graph using "fzero"
plot(r2_fzero,0,'bo')
plot(r3_fzero,0,'bo')

text(r1_fzero+0.3,0.02,'\leftarrow root')           %Add an arrow to show the root
...
x1_bisection = 1;                                   %Creating the variable x1 and x2 so tha we can use them in our method
x2_bisection = 6;
...
if f(x1_bisection)*f(x2_bisection)>0                %This is to make sure that we dont make a mistake when entering 
    ...
    r_bisection = (x1_bisection + x2_bisection)/2;  %We initiate the variable root
    err = abs(f(r_bisection));                      %We also define the error
    ...
    while err > epsilon                             %We start a while loop. The loop will not brake untill we reach our required precision
        i_bisection = i_bisection+1;                %Increas by one each time we go throw the loop(50)
   ...
   err = abs(f(r_bisection));                       %To calculate the error and when the err is lower then the precision thats when the
    end                                             %precision is met. Then we print the last value of "r_bisection" as that is our root
end
...
Method = {'Bisection';'False Position';'Secant';'Newton'};
...
mltable = table(Root,Iterations,Diference,'RowNames',Method)
